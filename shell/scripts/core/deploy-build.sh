...
deploy_config() {
    ...
    if [[ -f "$SYSTEM_CONFIG_FILE" ]]; then
        ...
        if grep -q "homelab" "$SYSTEM_CONFIG_FILE" || \
           ...
           [[ -f "$(dirname "$SYSTEM_CONFIG_FILE")/systemConfig/homelab-config.nix" ]] && \
           grep -q "homelab.*=.*true" "$(dirname "$SYSTEM_CONFIG_FILE")/systemConfig/homelab-config.nix" 2>/dev/null; then
            has_homelab=true
        fi
    fi
