...
func (lm *LayoutManager) GetAvailableDimensions() (int, int) {
    // EXPLICIT DIMENSIONING: Border nimmt (width-2) x (height-2)
    // Innerhalb der Border: Content-Bereich minus Border-Overhead

    bodyWidth := lm.width - 6  // (width-2) - 4 für Border-Overhead links/rechts

    // Border nimmt (height-2), dann minus Border-Overhead oben/unten = height-6
    // Dann Header/Footer-Overhead abziehen
    borderInnerHeight := lm.height - 6  // (height-2) - 4 für Border-Overhead

    fixedOverhead := 8  // Header (1) + Footer (1) + minimal spacing

    // RESPONSIVE: Overhead skaliert mit verfügbarem Raum
    scalingFactor := 0.15  // 15% Overhead für große Terminals
    minOverhead := 10      // Minimum für kleine Terminals

    totalOverhead := max(minOverhead, int(float64(borderInnerHeight) * scalingFactor))
    totalOverhead = max(totalOverhead, fixedOverhead)

    bodyHeight := borderInnerHeight - totalOverhead
    
    if bodyHeight < 10 {
        bodyHeight = 10  // Nie unter 10 Zeilen im Body-Bereich
    }
    if bodyHeight > borderInnerHeight - 6 {
        bodyHeight = borderInnerHeight - 6  // Nie zu nah an Border-Maximum
    }
