...
System discovery locks / secure storage of discovered facts.

## Config

- Read/write via `getModuleConfig "lock-manager"` (never hardcoded `config.core.…` paths).
...
This pack is owned by the module. Delete `lock-manager/` ⇒ pack gone from assistant discovery.
