...
let
  ...
  remoteHostsModule = import ../lib/remote-hosts.nix {
    inherit config lib pkgs getModuleApi;
    sshClientCfg = cfg;
  };
  keysModule = import ../lib/keys.nix {
    inherit config lib pkgs getModuleApi;
    sshClientCfg = cfg;
  };
  handlerModule = import ../handlers/connection-handler.nix {
    inherit config lib pkgs systemConfig getModuleConfig getModuleApi;
  };
  remoteHosts = remoteHostsModule.remoteHostsBash or "";
  keys = keysModule.keysBash or "";
  connectionHandler = handlerModule.connectionHandler or "";
in {
  # Interactive CLI for outbound SSH connections (ncc ssh client)
  clientMainScript = pkgs.writeScriptBin "ncc-ssh-client-main" ''
      #!${pkgs.bash}/bin/bash
          
      # Include remote-hosts, keys, connection handler
      ${remoteHosts}
      ${keys}
      ${connectionHandler}
