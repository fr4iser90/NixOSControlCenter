...
let
  ...
  serverOn = parent.enable or false;
  ...
  scriptModule = import ./scripts/main.nix {
    inherit config lib pkgs systemConfig getModuleConfig getModuleApi;
  };
  scriptPath = scriptModule.clientMainScript;
  sshClientTui =
    if tuiOn
    then (import ./ui/tui/default.nix {
      inherit config lib pkgs systemConfig scriptPath getModuleApi;
      sshClientCfg = cfg;
    }).tuiScript
    else null;
      ...
      "")
        case "$_ui" in
          ...
          tui)
            ...
            ;;
          *) exec ${scriptPath}/bin/ncc-ssh-client-main ;;
        esac
        ;;
      list|add|edit|delete|connect)
        exec ${scriptPath}/bin/ncc-ssh-client-main "$cmd" "''${@:2}"
        ;;
      help|-h|--help)
        cat <<EOF
...
in {
  config = mkMerge [
    # When server is on, ssh-manager/commands.nix owns domain + page (avoids conflicts)
    (mkIf (!serverOn) (cliRegistry.registerGuiDomain "ssh" {
      label = "SSH";
      description = "SSH server and client";
      enabled = cfg.enable or false;
      group = "features";
    }))
    (mkIf (!serverOn) (cliRegistry.registerGuiPage "ssh" ./ui/gui))
