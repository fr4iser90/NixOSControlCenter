...
SSH server on this host and optional outbound client tools.

## Config

- Read/write via `getModuleConfig "ssh-manager"` (never hardcoded `config.core.…` paths).
...
This pack is owned by the module. Delete `ssh-manager/` ⇒ pack gone from assistant discovery.
