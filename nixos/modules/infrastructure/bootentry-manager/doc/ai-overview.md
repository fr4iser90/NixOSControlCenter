...
Manage systemd-boot/GRUB boot entries.

## Config

- Read/write via `getModuleConfig "bootentry-manager"` (never hardcoded `config.core.…` paths).
...
This pack is owned by the module. Delete `bootentry-manager/` ⇒ pack gone from assistant discovery.
