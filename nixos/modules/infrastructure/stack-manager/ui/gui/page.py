"""Stacks — DomainPage kit (overview, containers, ports, domains, stacks)."""
...
class HomelabPage(DomainPage):
    ...
    def _load_overview(self) -> None:
        st = self.run_ncc(
            "stacks", "status", "--json", follow_target=True, log=False, show_error=False
        )
    ...
    def _fill_pipe_list(self, widget, verb: str, *, empty: str, fmt) -> None:
        widget.clear()
        proc = self.run_ncc(
            "stacks", verb, follow_target=True, log=False, show_error=False
        )
        lines = [
            ln.strip()
    ...
    def _load_stacks(self) -> None:
        self.stacks.clear()
        ls = self.run_ncc(
            "stacks", "list-stacks", follow_target=True, log=False, show_error=False
        )
