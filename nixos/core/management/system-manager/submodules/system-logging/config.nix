    ...
    lib.listToAttrs (lib.map (name: {
      ...
      value = import ./collectors/${name}.nix {
        ...
        currentLevel = reportLevels.${
          ...
          else config.systemConfig.core.management.system-manager.submodules.system-logging.defaultDetailLevel or "info"
        };
      };
    }) availableCollectors)
  );
in
  lib.mkMerge [
    ...
    (lib.mkIf (cfg.enable or true) {
      config.system.activationScripts.systemReport = {
        deps = [];
        text = let
        ...
        in ''
          ...
          ${ui.tables.keyValue "Detail Level" config.systemConfig.core.management.system-manager.submodules.system-logging.defaultDetailLevel}
    ...
    # Exportiere reportingConfig für andere Module
    {
      _module.args.reportingConfig = {
        ...
        currentLevel = reportLevels.${config.systemConfig.core.management.system-manager.submodules.system-logging.defaultDetailLevel};
      };
    }
  ]
