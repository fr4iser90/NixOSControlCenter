...
NIXEOF
            ...
            USER_CONFIG_NIX=""
            EVAL_ERR=$(mktemp)
            if USER_CONFIG_RAW=$(nix-instantiate --eval --strict \
                                    --argstr configFile "$NIXOS_DIR/system-config.nix" \
                                    "$TMP_EXTRACT" 2>"$EVAL_ERR"); then
              ...
              ${ui.messages.warning "Template sync: failed to extract users from system-config.nix"}
              if [ -s "$EVAL_ERR" ]; then
                sed 's/^/    /' "$EVAL_ERR" >&2
              fi
            fi
            rm -f "$TMP_EXTRACT" "$EVAL_ERR"
      ...
      if [ ! -f "$USER_CONFIG_NIX" ]; then
        if [ "$VERBOSE" = "true" ]; then
          ${ui.messages.info "Per-user config migration: $USER_CONFIG_NIX not found, skipping"}
        fi
      elif ! command -v nix-instantiate >/dev/null 2>&1; then
        ${ui.messages.warning "Per-user config migration: nix-instantiate not available, skipping"}
      else
        TMP_PER_USER=$(mktemp --suffix=.nix)
        cat > "$TMP_PER_USER" << 'NIXEOF'
{ configFile }:
let
  cfg = import configFile;
in
...
NIXEOF
        ...
        USER_JSON=""
        EVAL_ERR=$(mktemp)
        if USER_JSON=$(nix-instantiate --eval --strict --json \
                        --argstr configFile "$USER_CONFIG_NIX" \
                        "$TMP_PER_USER" 2>"$EVAL_ERR"); then
          : # success
        else
          USER_JSON=""
          ${ui.messages.warning "Per-user config migration: failed to evaluate user list"}
          if [ -s "$EVAL_ERR" ]; then
            sed 's/^/    /' "$EVAL_ERR" >&2
          fi
        fi
        rm -f "$TMP_PER_USER" "$EVAL_ERR"

        if [ -z "$USER_JSON" ] || [ "$USER_JSON" = "null" ] || [ "$USER_JSON" = "[]" ]; then
          if [ "$VERBOSE" = "true" ]; then
            ${ui.messages.info "Per-user config migration: no users found in core/base/user/config.nix"}
          fi
        else
          USERNAMES=$(echo "$USER_JSON" | jq -r '.[]' 2>/dev/null)
          if [ -z "$USERNAMES" ]; then
            ${ui.messages.warning "Per-user config migration: could not parse user list (jq failed)"}
            echo "    raw: $USER_JSON" >&2
          fi
...
# Generated by system-update migration
# User definition (role, shell) stays in core/base/user/config.nix
# This file is for package/override customizations only.
...
EOF
            PER_USER_MIGRATED=$((PER_USER_MIGRATED + 1))
            echo "  [CREATED] Per-user config: $USERNAME"
          done
        fi
      fi

      if [ "$PER_USER_MIGRATED" -gt 0 ]; then
        echo "  Migrated $PER_USER_MIGRATED user(s) to per-user configs"
      fi
    ...
    system.activationScripts.migratePerUserConfigs =
      ...
      in lib.optionalString (isSplitLayout && usernames != []) ''
        for USERNAME in ${lib.concatStringsSep " " usernames}; do
          ...
          mkdir -p "$USER_DIR"
          cat > "$USER_CFG" <<EOF_USERCFG
        # Per-User Config for: $USERNAME
        # Auto-generated by system-manager activation script.
        # User definition (role, shell) stays in core/base/user/config.nix.
        # This file is for package/override customizations only.
        ...
        EOF_USERCFG
          chmod 644 "$USER_CFG"
          chown root:root "$USER_CFG"
          echo "[migratePerUserConfigs] created $USER_CFG"
        done
      '';
  }
