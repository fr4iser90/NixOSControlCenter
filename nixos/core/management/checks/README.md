...
### Postbuild Checks
Postbuild checks run automatically after system activation via `system.activationScripts`.

## Configuration

Configure via `systemConfig.management.checks` in your `flake.nix`:

```nix
{
  systemConfig.management.checks = {
