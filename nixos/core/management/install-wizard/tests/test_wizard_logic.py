...
# Prefer prebuilt tree; else nix-build once for export-options.sh
_SCRIPT_ROOT = os.environ.get("SCRIPT_ROOT", "").strip()
if not _SCRIPT_ROOT:
    for cand in (
        Path("/tmp/ncc-install-tree"),
        Path(__file__).resolve().parents[1] / "result-script-tree",
    ):
        if (cand / "ui" / "gui" / "export-options.sh").is_file():
            _SCRIPT_ROOT = str(cand)
            break
if _SCRIPT_ROOT:
    os.environ.setdefault("SCRIPT_ROOT", _SCRIPT_ROOT)
