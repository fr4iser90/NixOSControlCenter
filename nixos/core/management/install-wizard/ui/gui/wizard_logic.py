...
class InstallOptions:
    ...
    def __init__(self) -> None:
        ...
        self.install_starters: List[str] = []
        self.device_blueprint_map: Dict[str, str] = {}
...
def _export_options_script() -> Path:
    """Resolve export-options.sh from SCRIPT_ROOT (store) or env override."""
    env = (os.environ.get("NCC_EXPORT_OPTIONS") or "").strip()
    if env and Path(env).is_file():
        return Path(env)
    root = (os.environ.get("SCRIPT_ROOT") or "").strip()
    if root:
        cand = Path(root) / "ui" / "gui" / "export-options.sh"
        if cand.is_file():
            return cand
    # Sibling of this file only if present (legacy); prefer store tree
    legacy = Path(__file__).resolve().parent / "export-options.sh"
    if legacy.is_file():
        return legacy
    raise FileNotFoundError(
        "export-options.sh not found — set SCRIPT_ROOT to install script tree "
        "or NCC_EXPORT_OPTIONS to the store path"
    )


def load_options() -> InstallOptions:
    """Source setup-options via export-options.sh (SSOT)."""
    ...
    for raw in proc.stdout.splitlines():
        ...
        elif section == "INSTALL_STARTERS":
            opts.install_starters.append(line)
        elif section == "DEVICE_TARGETS":
            opts.device_presets.append(line)
        elif section == "DEVICE_BLUEPRINT_MAP":
            k, _, v = line.partition("=")
            if k:
                opts.device_blueprint_map[k] = v
