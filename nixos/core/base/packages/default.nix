...
let
  ...
  # Load preset if set
  presetConfig = if (systemConfig.core.base.packages or {}).preset or null != null then
    import (./presets + "/${(systemConfig.core.base.packages or {}).preset}.nix")
  else null;
        ...
        in
          if matchingFeatures != {} && value == true then
            let
              ...
              additionalFeatures = if featureName == "qemu-vm" && systemConfig.core.management.system-manager.systemType == "desktop" then
                [ "virt-manager" ]
              else [];
            in
  ...
  # Get packages config
  packagesCfg = systemConfig.core.base.packages or {};

  # Check if old structure (Set) is present
  isOldStructure = packagesCfg.packageModules or null != null 
    && !lib.isList (packagesCfg.packageModules or null);

  # Convert old structure if needed
  migratedFeatures = if isOldStructure then
    convertOldPackageModules packagesCfg.packageModules
  else [];
  ...
  allFeatures = 
    ...
    ++ (packagesCfg.additionalPackageModules or [])
    ++ (warnMigration migratedFeatures)
    ++ (if !isOldStructure then (packagesCfg.packageModules or []) else []);
  ...
  validFeatures = lib.filter (feature:
    ...
    in lib.elem systemConfig.core.management.system-manager.systemType (meta.systemTypes or [])
  ) uniqueFeatures;
