...
Hostname, NetworkManager DNS, firewall basics.

## Config

- Read/write via `getModuleConfig "network"` (never hardcoded `config.core.…` paths).
...
This pack is owned by the module. Delete `network/` ⇒ pack gone from assistant discovery.
